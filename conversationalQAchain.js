...
const { ChatPromptTemplate, SystemMessagePromptTemplate, HumanMessagePromptTemplate, PromptTemplate } = require("@langchain/core/prompts");
...
class BibleBuddyQA {
    constructor() {
        ...
        this.systemTemplate = `
        Use the following pieces of context to try to answer the users question. 
        If you don't know the answer, politely ask for clarification or a followup question! I repeat, only answer based on the context or to followup on a previous query. If the answer is in the {bible} but not in the context provided please answer using that known scripture.
        ...
        this.prompt = ChatPromptTemplate.fromMessages(this.messages);
        this.qa_template = `Use the following pieces of context to try to answer the users question. 
    ...
    async createChain(bible) {
        ...
        return ConversationalRetrievalQAChain.fromLLM(
            ...
            retriever,
            {
                qaPrompt: PromptTemplate.fromTemplate(this.qa_template),
    ...
    async getAnswer(inputData) {
        ...
        const question = cleanText(inputData.prompt);
        const lastResponse = cleanText(inputData.last_response);
        const lastPrompt = cleanText(inputData.last_prompt);
